...
const DEFAULT_PRODUCT_NAME = 'Tempo Testnet Session Photo';
...
function createServerConfig() {
  ...
  return {
    ...
    recipientPrivateKey: (process.env.RECIPIENT_PRIVATE_KEY || '').trim() || null,
...
const recipientAccount = config.recipientPrivateKey
  ? privateKeyToAccount(config.recipientPrivateKey)
  : null;
if (recipientAccount) {
  config.recipient = recipientAccount.address;
}
...
const sessionPhoto = mppx.session({
  amount: config.amount,
  unitType: 'token',
  description: `${config.productName} via Tempo testnet session`,
});
...
async function handleRequest(request) {
  ...
  const result = await sessionPhoto(request);
...
server.listen(config.port, config.host, () => {
  ...
  console.error(
    [
      `MPP session demo server listening on ${serverUrl}`,
      ...
      `currency: ${config.currency}`,
      `realm: ${realm}`,
      `recipient account: ${recipientAccount ? 'configured' : 'not configured (close-session will fail)'}`,
