...
When a task touches wallet setup, funding, policy, transfers, approvals, or backups, load and follow the installed `agentpay-sdk` skill.
...
- if wallet metadata is unavailable and the user is trying to use the wallet, tell them to run `agentpay admin setup` locally on macOS; on Linux explain that managed wallet bootstrap is not implemented there yet
...
- prefer `agentpay admin tui` for policy changes on macOS
